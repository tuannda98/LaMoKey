...
#define POPUP_ABOUT_LAMOKEY 1010
#define POPUP_LAMOKEY_EXIT 2000
...
map<UINT, LPCTSTR> menuData = {
	...
	{POPUP_ABOUT_LAMOKEY, _T("Giới thiệu LaMoKey")},
	{POPUP_LAMOKEY_EXIT, _T("Thoát")},
};
	...
	case WM_TRAYMESSAGE: {
		...
		if (lParam == WM_LBUTTONUP) {
			...
			case POPUP_CONTROL_PANEL:
				AppDelegate::getInstance()->onControlPanel();
				break;
			case POPUP_ABOUT_LAMOKEY:
				AppDelegate::getInstance()->onLaMoKeyAbout();
				break;
			case POPUP_LAMOKEY_EXIT:
...
void SystemTrayHelper::createPopupMenu() {
	...
	AppendMenu(popupMenu, MF_UNCHECKED, POPUP_ABOUT_LAMOKEY, menuData[POPUP_ABOUT_LAMOKEY]);
	AppendMenu(popupMenu, MF_SEPARATOR, 0, 0);
	AppendMenu(popupMenu, MF_UNCHECKED, POPUP_LAMOKEY_EXIT, menuData[POPUP_LAMOKEY_EXIT]);
