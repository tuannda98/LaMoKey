...
void MacroDialog::saveAndReload() {
	...
	LaMoKeyHelper::setRegBinary(_T("macroData"), macroData.data(), (int)macroData.size());
...
void MacroDialog::onAddMacroButton() {
	...
	if (name.compare(L"") == 0 || content.compare(L"") == 0) {
		MessageBox(hDlg, _T("Bạn hãy nhập từ cần gõ tắt!"), _T("LaMoKey"), MB_OK);
		return;
	}
...
void MacroDialog::onDeleteMacroButton() {
	...
	if (name.compare(L"") == 0) {
		MessageBox(hDlg, _T("Bạn hãy chọn từ cần xoá!"), _T("LaMoKey"), MB_OK);
		return;
	}
