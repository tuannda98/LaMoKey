...
void MainControlDialog::initDialog() {
    ...
    wsprintfW(titleBuffer, title, LaMoKeyHelper::getVersionString().c_str());
    SetWindowText(hDlg, titleBuffer);

    //input type
    comboBoxInputType = GetDlgItem(hDlg, IDC_COMBO_INPUT_TYPE);
    vector<LPCTSTR>& inputType = LaMoKeyManager::getInputType();
    ...
    vector<LPCTSTR>& tableCode = LaMoKeyManager::getTableCode();
    ...
    checkTempOffLaMoKey = GetDlgItem(hTabPage1, IDC_CHECK_TEMP_OFF_OPEN_KEY);
    createToolTip(checkTempOffLaMoKey, IDS_STRING_TEMP_OFF_LAMOKEY);
...
INT_PTR MainControlDialog::eventProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        case IDBUTTON_OK:
            ...
            break;
        case IDBUTTON_EXIT:
            AppDelegate::getInstance()->onLaMoKeyExit();
            break;
        case ID_BTN_DEFAULT: {
        ...
        case IDC_BUTTON_GO_SOURCE_CODE:
            ShellExecute(NULL, _T("open"), _T("https://github.com/tuyenvm/LaMoKey"), NULL, NULL, SW_SHOWNORMAL);
            break;
        default:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                _lastKeyCode = LaMoKeyManager::_lastKeyCode;
                if (_lastKeyCode > 0) {
                    LaMoKeyManager::_lastKeyCode = 0;
    ...
    case WM_NOTIFY: {
        ...
        case NM_RETURN: {
            ...
            if (link->hdr.idFrom == IDC_SYSLINK_HOME_PAGE)
                ...
                ShellExecute(NULL, _T("open"), _T("https://www.facebook.com/LaMoKeyVN"), NULL, NULL, SW_SHOWNORMAL);
...
void MainControlDialog::fillData() {
    ...
    SendMessage(checkTempOffLaMoKey, BM_SETCHECK, vTempOffLaMoKey ? 1 : 0, 0);
    ...
    //tab info
    wchar_t buffer[256];
    wsprintfW(buffer, _T("Phiên bản %s cho Windows - Ngày cập nhật: %s"), LaMoKeyHelper::getVersionString().c_str(), _T(__DATE__));
...
void MainControlDialog::onComboBoxSelected(const HWND& hCombobox, const int& comboboxId) {
    if (hCombobox == comboBoxInputType) {
        ...
        if (vRememberCode) {
            setAppInputMethodStatus(LaMoKeyHelper::getFrontMostAppExecuteName(), vLanguage | (vCodeTable << 1));
...
void MainControlDialog::onCheckboxClicked(const HWND& hWnd) {
    ...
    if (hWnd == checkCtrl) {
        ...
        LaMoKeyHelper::registerRunOnStartup(vRunWithWindows);
    }
        ...
        if (val)
            LaMoKeyManager::createDesktopShortcut();
    }
    ...
    else if (hWnd == checkTempOffLaMoKey) {
        val = (int)SendMessage(hWnd, BM_GETCHECK, 0, 0);
        APP_SET_DATA(vTempOffLaMoKey, val ? 1 : 0);
    }
...
void MainControlDialog::onUpdateButton() {
    ...
    string newVersion;
    if (LaMoKeyManager::checkUpdate(newVersion)) {
        WCHAR msg[256];
        wsprintf(msg,
            TEXT("LaMoKey Có phiên bản mới (%s), bạn có muốn cập nhật không?"),
        ...
        if (msgboxID == IDYES) {
            ...
            wsprintf(path, TEXT("%s\\LaMoKeyUpdate.exe"), path);
        ...
        MessageBox(hDlg, _T("Bạn đang dùng phiên bản mới nhất!"), _T("LaMoKey Update"), MB_OK);
    }
    EnableWindow(hUpdateButton, true);
}

void MainControlDialog::requestRestartAsAdmin() {
    LaMoKeyHelper::registerRunOnStartup(false);
    if (vRunAsAdmin && !IsUserAnAdmin()) {
        int msgboxID = MessageBox(
            hDlg,
            _T("Bạn cần phải khởi động lại LaMoKey để kích hoạt chế độ Admin!\nBạn có muốn khởi động lại LaMoKey không?"),
            _T("LaMoKey"),
            MB_ICONEXCLAMATION | MB_YESNO
        );
        if (msgboxID == IDYES) {
            PostQuitMessage(0);
            ShellExecute(0, L"runas", LaMoKeyHelper::getFullPath().c_str(), 0, 0, SW_SHOWNORMAL);
        }
    }
    else {
        LaMoKeyHelper::registerRunOnStartup(vRunWithWindows);
