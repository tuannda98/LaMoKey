...
int AppDelegate::run(HINSTANCE hInstance) {
	...
	while (GetMessage(&msg, nullptr, 0, 0))	{
		if (msg.message == WM_KEYDOWN) {
			LaMoKeyManager::_lastKeyCode = (UINT16)msg.wParam;
		}
		if (!isDialogMsg(msg)) {
...
void AppDelegate::onDefaultConfig() {
	...
	APP_SET_DATA(vTempOffLaMoKey, 0);
...
void AppDelegate::onToggleVietnamese() {
	...
	if (vUseSmartSwitchKey) {
		string& exe = LaMoKeyHelper::getLastAppExecuteName();
...
void AppDelegate::onLaMoKeyAbout() {
	if (aboutDialog == NULL) {
...
void AppDelegate::onLaMoKeyExit() {
	LaMoKeyManager::freeEngine();
