...
void LaMoKeyInit() {
	...
	APP_GET_DATA(vTempOffLaMoKey, 0);
	...
	DWORD macroDataSize;
	BYTE* macroData = LaMoKeyHelper::getRegBinary(_T("macroData"), macroDataSize);
	initMacroMap((Byte*)macroData, (int)macroDataSize);

	//init and load smart switch key data
	DWORD smartSwitchKeySize;
	BYTE* data = LaMoKeyHelper::getRegBinary(_T("smartSwitchKey"), smartSwitchKeySize);
...
void saveSmartSwitchKeyData() {
	...
	LaMoKeyHelper::setRegBinary(_T("smartSwitchKey"), savedSmartSwitchKeyData.data(), (int)savedSmartSwitchKeyData.size());
}
...
static void SendBackspace() {
	...
	if (vSupportMetroApp && LaMoKeyHelper::getLastAppExecuteName().compare("ApplicationFrameHost.exe") == 0) {//Metro App
...
static void SendNewCharString(const bool& dataFromMacro = false) {
	...
	LaMoKeyHelper::setClipboardText((LPCTSTR)_newCharString.data(), _newCharSize + 1, CF_UNICODETEXT);
...
LRESULT CALLBACK keyboardHookProcess(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	if ((wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) && !_isFlagKey && _keycode != 0) {
		...
		if (_lastFlag == 0 || _lastFlag < _flag)
			...
			if (vTempOffLaMoKey && !_hasJustUsedHotKey && _lastFlag & MASK_ALT) {
				vTempOffEngine();
			}
			_lastFlag = _flag;
		...
		if (pData->code == vDoNothing) { //do nothing
			...
			if (vFixRecommendBrowser && pData->extCode != 4) {
				if (vFixChromiumBrowser && 
					std::find(_chromiumBrowser.begin(), _chromiumBrowser.end(), LaMoKeyHelper::getLastAppExecuteName()) != _chromiumBrowser.end()) {
...
VOID CALLBACK winEventProcCallback(HWINEVENTHOOK hWinEventHook, DWORD dwEvent, HWND hwnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime) {
	...
	if (vUseSmartSwitchKey || vRememberCode) {
		string& exe = LaMoKeyHelper::getFrontMostAppExecuteName();
