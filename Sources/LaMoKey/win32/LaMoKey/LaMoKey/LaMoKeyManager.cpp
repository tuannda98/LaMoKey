...
extern void LaMoKeyInit();
extern void LaMoKeyFree();

unsigned short  LaMoKeyManager::_lastKeyCode = 0;

vector<LPCTSTR>& LaMoKeyManager::getInputType() {
	return _inputType;
}

vector<LPCTSTR>& LaMoKeyManager::getTableCode() {
	return _tableCode;
}

void LaMoKeyManager::initEngine() {
	LaMoKeyInit();
}

void LaMoKeyManager::freeEngine() {
	LaMoKeyFree();
}
...
bool LaMoKeyManager::checkUpdate(string& newVersion) {
	...
	DWORD currentVersionCode = LaMoKeyHelper::getVersionNumber();
	currentVersionCode = shiftVersion(currentVersionCode);

	return newVersionCode > currentVersionCode;
}

void LaMoKeyManager::createDesktopShortcut() {
	CoInitialize(NULL);
	...
	if (SUCCEEDED(hres)) {
		wstring path = LaMoKeyHelper::getFullPath();
		pShellLink->SetPath(path.c_str());
		pShellLink->SetDescription(_T("LaMoKey - Bộ gõ Tiếng Việt"));
		...
		if (SUCCEEDED(hres)) {
			...
			wsprintf(savePath, _T("%s\\LaMoKey.lnk"), desktopPath);
