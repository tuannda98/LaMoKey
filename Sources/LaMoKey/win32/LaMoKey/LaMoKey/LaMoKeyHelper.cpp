...
Github: https://github.com/tuyenvm/LaMoKey
Fanpage: https://www.facebook.com/LaMoKeyVN

This file is belong to the LaMoKey project, Win32 version
which is released under GPL license.
You can fork, modify, improve this program. If you
redistribute your new version, it MUST be open source.
-----------------------------------------------------------*/
#include "LaMoKeyHelper.h"
#include <stdarg.h>
#include <Urlmon.h>
#include <fstream>
#include <sstream>
...
static LPCTSTR sk = TEXT("SOFTWARE\\TuyenMai\\LaMoKey");
static HKEY hKey;
...
static HANDLE _proc;
static string _exeNameUtf8 = "TheLaMoKeyProject";
...
int CF_LAMOKEY = RegisterClipboardFormat(_T("LaMoKey Format"));

void LaMoKeyHelper::openKey() {
	LONG nError = RegLaMoKeyEx(HKEY_CURRENT_USER, sk, NULL, KEY_ALL_ACCESS, &hKey);
...
void LaMoKeyHelper::setRegInt(LPCTSTR key, const int & val) {
	openKey();
	RegSetValueEx(hKey, key, 0, REG_DWORD, (LPBYTE)&val, sizeof(val));
	RegCloseKey(hKey);
}

int LaMoKeyHelper::getRegInt(LPCTSTR key, const int & defaultValue) {
	openKey();
...
void LaMoKeyHelper::setRegBinary(LPCTSTR key, const BYTE * pData, const int & size) {
	openKey();
	RegSetValueEx(hKey, key, 0, REG_BINARY, pData, size);
	RegCloseKey(hKey);
}

BYTE * LaMoKeyHelper::getRegBinary(LPCTSTR key, DWORD& outSize) {
	openKey();
	if (_regData) {
		delete[] _regData;
		_regData = NULL;
...
void LaMoKeyHelper::registerRunOnStartup(const int& val) {
	if (val) {
		if (vRunAsAdmin) {
			string path = wideStringToUtf8(getFullPath());
			char buff[MAX_PATH];
			sprintf_s(buff, "schtasks /create /sc onlogon /tn LaMoKey /rl highest /tr \"%s\" /f", path.c_str());
			WinExec(buff, SW_HIDE);
		} else {
			RegLaMoKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
			wstring path = getFullPath();
			RegSetValueEx(hKey, _T("LaMoKey"), 0, REG_SZ, (byte*)path.c_str(), ((DWORD)path.size() + 1) * sizeof(TCHAR));
			RegCloseKey(hKey);
		}
	} else {
		RegLaMoKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
		RegDeleteValue(hKey, _T("LaMoKey"));
		RegCloseKey(hKey);
		WinExec("schtasks /delete  /tn LaMoKey /f", SW_HIDE);
	}
}

LPTSTR LaMoKeyHelper::getExecutePath() {
	if (!_hasGetPath) {
...
string& LaMoKeyHelper::getFrontMostAppExecuteName() {
	...
	if (wcscmp(_exeName, _T("LaMoKey64.exe")) == 0 ||
		wcscmp(_exeName, _T("LaMoKey32.exe")) == 0 || 
...
string & LaMoKeyHelper::getLastAppExecuteName() {
	if (!vUseSmartSwitchKey)
		return getFrontMostAppExecuteName();
	return _exeNameUtf8;
}

wstring LaMoKeyHelper::getFullPath() {
...
wstring LaMoKeyHelper::getClipboardText(const int& type) {
...
void LaMoKeyHelper::setClipboardText(LPCTSTR data, const int & len, const int& type) {
...
bool LaMoKeyHelper::quickConvert() {
...
DWORD LaMoKeyHelper::getVersionNumber() {
...
wstring LaMoKeyHelper::getVersionString() {
...
wstring LaMoKeyHelper::getContentOfUrl(LPCTSTR url){
	WCHAR path[MAX_PATH];
	GetTempPath2(MAX_PATH, path);
	wsprintf(path, TEXT("%s\\_LaMoKey.tempf"), path);
