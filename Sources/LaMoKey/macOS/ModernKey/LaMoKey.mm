...
extern "C" {
    ...
    void LaMoKeyInit() {
        ...
        LOAD_DATA(vTempOffLaMoKey, vTempOffLaMoKey);
    ...
    void queryFrontMostApp() {
        if ([[[NSWorkspace sharedWorkspace] frontmostApplication].bundleIdentifier compare:LAMOKEY_BUNDLE] != 0) {
     ...
     * MAIN Callback.
     */
    CGEventRef LaMoKeyCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon) {
        ...
        if (type == kCGEventKeyDown) {
            ...
            if (_lastFlag == 0 || _lastFlag < _flag) {
                ...
                if (vTempOffLaMoKey && !_hasJustUsedHotKey && _lastFlag & kCGEventFlagMaskCommand) {
                    vTempOffEngine();
                }
                _lastFlag = 0;
