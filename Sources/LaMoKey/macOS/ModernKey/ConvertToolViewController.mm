...
- (IBAction)onConvertButton:(id)sender {
    if ([LaMoKeyManager quickConvert]) {
        if (!convertToolDontAlertWhenCompleted) {
            [LaMoKeyManager showMessage: self.view.window message:@"Chuyển mã thành công!" subMsg:@"Kết quả đã được lưu trong clipboard."];
        }
    } else {
        [LaMoKeyManager showMessage: self.view.window message:@"Không có dữ liệu trong clipboard!" subMsg:@"Hãy sao chép một đoạn text để chuyển đổi!"];
    }
}
