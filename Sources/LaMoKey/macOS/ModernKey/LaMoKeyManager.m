...
extern void LaMoKeyInit(void);

extern CGEventRef LaMoKeyCallback(CGEventTapProxy proxy,
                                  CGEventType type,
                                  CGEventRef event,
                                  void *refcon);
...
@interface LaMoKeyManager ()

@end

@implementation LaMoKeyManager {
...
+(void)showUpdateMessage:(NSWindow*)parent needUpdating:(BOOL)needUpdating newVersion:(NSString*)versionString {
    NSAlert *alert = [[NSAlert alloc] init];
    [alert setMessageText:(needUpdating ? [NSString stringWithFormat:@"LaMoKey Có phiên bản mới (%@), bạn có muốn cập nhật không?", versionString] : @"Bạn đang dùng phiên bản mới nhất!")];
    [alert setInformativeText:(needUpdating ? @"Bấm 'Có' để cập nhật LaMoKey." : @"")];
...
+(NSString*)getApplicationSupportFolder {
    ...
    return [NSString stringWithFormat:@"%@/LaMoKey", applicationSupportDirectory];
}

+(NSString*)getUpdateBundlePath {
    NSString *currentpath = [[NSBundle mainBundle] bundlePath];
    return [NSString stringWithFormat:@"%@/Contents/Library/LoginItems/LaMoKeyUpdate.app", currentpath];
}
@end
