...
int vTempOffLaMoKey = 0; //new on version 2.0
...
-(void)askPermission {
    ...
    [alert setMessageText: [NSString stringWithFormat:@"LaMoKey cần bạn cấp quyền để có thể hoạt động!"]];
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (![LaMoKeyManager initEventTap]) {
    ...
    if (!dontCheckUpdate)
        [LaMoKeyManager checkNewVersion:nil callbackFunc:nil];
...
-(void)loadDefaultConfig {
    ...
    vTempOffLaMoKey = 0;[[NSUserDefaults standardUserDefaults] setInteger:vTempOffLaMoKey forKey:@"vTempOffLaMoKey"];
...
-(void)setRunOnStartup:(BOOL)val {
    CFStringRef appId = (__bridge CFStringRef)@"com.tuyenmai.LaMoKeyHelper";
...
-(void)onConvertTool {
    ...
    //[LaMoKeyManager showDockIcon:YES];
...
-(void)onQuickConvert {
    if ([LaMoKeyManager quickConvert]) {
        if (!convertToolDontAlertWhenCompleted) {
            [LaMoKeyManager showMessage: nil message:@"Chuyển mã thành công!" subMsg:@"Kết quả đã được lưu trong clipboard."];
        }
    } else {
        [LaMoKeyManager showMessage: nil message:@"Không có dữ liệu trong clipboard!" subMsg:@"Hãy sao chép một đoạn text để chuyển đổi!"];
    }
}

-(void) onControlPanelSelected {
    if (_mainWC == nil) {
        _mainWC = [[NSStoryboard storyboardWithName:@"Main" bundle:nil] instantiateControllerWithIdentifier:@"LaMoKey"];
    }
    //[LaMoKeyManager showDockIcon:YES];
...
#pragma mark Reset LaMoKey after mac computer awake
-(void)receiveWakeNote: (NSNotification*)note {
    [LaMoKeyManager initEventTap];
}

-(void)receiveSleepNote: (NSNotification*)note {
    [LaMoKeyManager stopEventTap];
}
...
-(void)activeAppChanged: (NSNotification*)note {
    if (vUseSmartSwitchKey && [LaMoKeyManager isInited]) {
        OnActiveAppChanged();
    }
}
